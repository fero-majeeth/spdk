...
virtqueue_dequeue_burst_rx_packed(struct virtqueue *vq, void **rx_pkts, uint32_t *len, uint16_t num)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        /* Verify descriptor is used */
        if (spdk_unlikely(!desc_is_used(&desc[used_idx], vq))) {
            break;
        }

        id = used_idx; /* our request index was the head index when request started */
        ...
        if (spdk_unlikely(dxp->cookie == NULL || dxp->ndescs == 0)) {
            break;
        }

        uint16_t ndescs = dxp->ndescs;
        uint16_t last_idx = (used_idx + ndescs - 1) % vq->vq_nentries;

        uint8_t *status = (uint8_t *)(uintptr_t)desc[last_idx].addr;

        uint16_t data_idx = (used_idx + 1) % vq->vq_nentries;
        len[i] = desc[data_idx].len;
        ...
        vq->vq_free_cnt += ndescs; /* reclaim chain */
        vq->vq_used_cons_idx += ndescs; /* advance past chain */
        dxp->cookie = NULL;

        /* Handle wrap-around of the used index and toggle wrap counter */
...
virtqueue_req_flush_packed(struct virtqueue *vq)
{
	...
	head_idx = dxp->head_idx;
        ...
        /* clear next flag in last descriptor */
        desc[last_idx].flags &= ~VRING_DESC_F_NEXT;

	desc[last_idx].flags |= vq->vq_packed.cached_flags;
	desc[head_idx + 1].flags |= vq->vq_packed.cached_flags;
                              ...
                              enum spdk_virtio_desc_type desc_type)
{
	...
	uint16_t id;
	uint16_t flag_desc = VRING_DESC_F_NEXT;
	...
	for (i = 0; i < iovcnt; ++i) {
		...
		while (processed_length < iovec_length) {
			...
			desc[desc_idx].id = 0;
			desc[desc_idx].flags = desc_type | flag_desc;
